...
class Model(nn.Module):
    ...
    def din_inner_cross(self, history_nids_embedding, impr_nid_embedding, history_len):
        """
        history_nids_embedding: [B, L, D]
        impr_nid_embedding: [B, D]
        history_len: [B]
        """
        B, L, D = history_nids_embedding.size()
        din_att_raw_score = torch.multiply(history_nids_embedding, impr_nid_embedding.view(B, 1, D))
        mask = (self.L_tensor.view(1, -1) >= history_len.view(-1, 1)).view(B, L, 1)
        din_att_raw_score = din_att_raw_score.masked_fill(mask, -1e9)
        din_att_score = F.softmax(din_att_raw_score, dim=1)
        return torch.sum(din_att_score * history_nids_embedding, dim=1)
