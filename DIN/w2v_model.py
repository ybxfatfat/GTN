...
class Model(nn.Module):
    def __init__(self, config):
        ...
        if config.use_pretrain:
            ...
            self.w2v_embedding = nn.Embedding(config.vocab_size, config.embedding_dim)
        ...
        d_model, n_head = 256, 4
        self.pe = PositionalEncoding(config.embedding_dim, 0.1, config.text_len)
        self.multi_head_att = MultiHeadedAttention(self.config.embedding_dim, d_model, n_head, self.config.text_len)
        self.transform = nn.Sequential(
            nn.Dropout(p=0.2),
            nn.Linear(d_model, self.config.embedding_dim)
        )
        ...
        self.din_att = nn.Sequential(
            nn.Linear(12 * config.embedding_dim, 512),
            nn.LeakyReLU(inplace=True),
            nn.Linear(512, 128),
            nn.LeakyReLU(inplace=True),
            nn.Linear(128, 1)
    ...
    def load_pretrain_embeddings(self, config):
        ...
        self.w2v_embedding = nn.Embedding.from_pretrained(torch.FloatTensor(np.load(config.w2v_embedding_path)), freeze=False)
        
    def sum_pooling(self, tensor, length):
        """
        tensor: [B, L, T, D] or [B, T, D]
        length: [B, L] or [B]
        """
        if len(tensor.size()) == 4:
            B, L, T, D = tensor.size()
            # length: [B, L]
            mask = (self.T_tensor.view(1, 1, -1) >= length.view(B, L, 1)).view(B, L, T, 1)
            tensor = tensor.masked_fill(mask, 0)
            return torch.sum(tensor, dim=-2) # B, L, D
        else:
            B, T, D = tensor.size()
            # length: [B]
            mask = (self.T_tensor.view(1, -1) >= length.view(-1, 1)).view(B, T, 1)
            tensor = tensor.masked_fill(mask, 0)
            return torch.sum(tensor, dim=-2)   # B, D
        
    def self_attention(self, tensor, length):
        """
        tensor: [B, L, T, D] or [B, T, D]
        length: [B, L] or [B]
        """
        B, L, T, D = None, None, None, None
        if len(tensor.size()) == 4:
            B, L, T, D = tensor.size()
            tensor = tensor.view(-1, T, D)
            length = length.view(-1) # B * L
        else:
            B, T, D = tensor.size()
        out = self.pe(tensor)
        out = self.multi_head_att(out, length)  # [B*L, T, 512] or [B, T , 512]
        out = self.transform(out)  # [B*L, T, D] or [B, T , D]
        # 再次attention
        att_raw_score = self.att(out)  # B, T
        mask = (self.T_tensor.view(1, -1) >= length.view(-1, 1)).view(-1, T, 1) # B, T, 1
        att_raw_score = att_raw_score.masked_fill(mask, -1e9)
        att_score = F.softmax(att_raw_score, dim=1)  # B, T, 1
        out = torch.sum(att_score * out, dim=1) # [B, D] or [B*L, D]
        # mask = (self.T_tensor.view(1, -1) >= length.view(-1, 1)).view(-1, T, 1) # B, T, 1
    ...
    def din_cross(self, history_nids_embedding, impr_nid_embedding, history_len):
        """
        history_nids_embedding: [B, L, D]
        impr_nid_embedding: [B, D]
        history_len: [B]
        """
        B, L, D = history_nids_embedding.size()
        combine_input = torch.cat([
            history_nids_embedding,
            impr_nid_embedding.view(B, 1, D).repeat(1, L, 1),
            history_nids_embedding + impr_nid_embedding.view(B, 1, D),
            history_nids_embedding * impr_nid_embedding.view(B, 1, D)
        ], dim=-1)
        att_raw_score = self.din_att(combine_input) # [B, L, 1]
        mask = (self.L_tensor.view(1, -1) >= history_len.view(-1, 1)).view(B, L, 1)
        # 归一化
        att_raw_score = att_raw_score.masked_fill(mask, -1e9)
        att_score = F.softmax(att_raw_score, dim=1)
        out = torch.sum(att_score * history_nids_embedding, dim=1)
        # 不归一化
        # att_raw_score = att_raw_score.masked_fill(mask, 0)
        # out = torch.sum(att_raw_score * history_nids_embedding, dim=1)
        return out
    ...
    def forward(self, batch_datas):
        ...
        history_nids_words: [B, config.history_len, config.text_len]
        history_words_len: [B, config.history_len]
        history_len: [B]
        category_ids: [B, config.history_len]
        topic_ids: [B, config.history_len]
        impr_nid_words: [B, config.text_len]
        impr_nid_words_len: [B]
        impr_category_id: [B]
        impr_topic_id: [B]
        week: [B]
        hour: [B]
        ...
        uid, impr_nid, history_nids, history_nids_words, history_words_len, history_len, category_ids, topic_ids, \
            impr_nid_words, impr_nid_words_len, impr_category_id, impr_topic_id, week, hour, relative_days, relative_hours, relative_seconds, label = batch_datas
        
        uid_embedding = self.uid_embedding(uid)
        history_nids_words_embedding = self.w2v_embedding(history_nids_words)
        history_nids_embedding = self.nid_embedding_structual(history_nids)
        category_embedding = self.category_embedding(category_ids)
        topic_embedding = self.topic_embedding(topic_ids)
        impr_nid_embedding = self.nid_embedding_structual(impr_nid)
        impr_nid_words_embedding = self.w2v_embedding(impr_nid_words)
        ...
        if self.config.agg_method == 'pooling':
            history_nids_words_embedding = self.sum_pooling(history_nids_words_embedding, history_words_len)
            impr_nid_words_embedding = self.sum_pooling(impr_nid_words_embedding, impr_nid_words_len)
        else:
            history_nids_words_embedding = self.self_attention(history_nids_words_embedding, history_words_len)
            impr_nid_words_embedding = self.self_attention(impr_nid_words_embedding, impr_nid_words_len)
            
        history_nids_embedding = history_nids_embedding + history_nids_words_embedding
        impr_nid_embedding = impr_nid_embedding + impr_nid_words_embedding
        ...
        history_embedding = self.din_cross(history_embedding, impr_embedding, history_len)
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, dropout, text_len):
        super(PositionalEncoding, self).__init__()
        self.dropout = nn.Dropout(p=dropout)
        # Compute the positional encodings once in log space.
        pe = torch.zeros(text_len, d_model)
